...
class CartridgeMomentPrice(Entity):
    id                      = helpers.PrimaryKey(int, auto=True)
    cartridge_id            = helpers.Required(str, 64, index=True, unique=True)
...
def initialize_moment():
    expression = "a * moments + b"
    m = MomentPriceModel.select(lambda r: r.expression == expression).first()
    if m is None:
        m = MomentPriceModel(
            expression = expression # simple linear
        )
    ...
    for cartridge_id in listdir(riv_get_cartridges_path()):
        try:
            ...
            if helpers.count(c for c in CartridgeMomentPrice if c.cartridge_id == cartridge_id) > 0:
                raise Exception(f"CartridgeMomentPrice already exists")
        ...
        if cartridge_info_json['name'].lower() == 'antcopter':
            ...
            try:
                ...
                if helpers.count(c for c in Achievement if c.id == get_achievement_id(expression, cartridge_id)) > 0:
                    raise Exception(f"Achievement already exists")
                a = Achievement(
